		...
		if (!contribution) {
			throw new Error(
				`UI contribution "${contributionId}" not found in registry.\n` +
					`Action: Register "${contributionId}" via registerSidebarPanel/registerStatusBarItem/registerTabContent before mounting, ` +
					`or check the id for a typo (getSidebarPanels()/getStatusBarItems()/getTabContents() lists registered ids).`
			);
		}
