...
test.describe('FileOrigin & Registry Tests', () => {
  ...
  test('file.openPath command converts supported raw paths and handles unsupported paths cleanly', async ({ page }) => {
    await mockIconThemes(page);
    await page.goto('/');
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        try {
          await appState.commands.execute('file.openPath', 'unrooted-relative.md');
        } catch (e) {
          relativeThrew = true;
        }

        // 2. Scheme-less target without rootOrigin - absolute POSIX path
        await appState.commands.execute('file.openPath', '/var/log/app.log');

        // 3. Scheme-less target without rootOrigin - Windows path
        await appState.commands.execute('file.openPath', 'C:\\Users\\notes\\todo.txt');

        // 4. URI target
        await appState.commands.execute('file.openPath', 'file:///etc/hosts');

        // 5. Scheme-less target with rootOrigin
        appState.workspace.rootOrigin = { scheme: 'browser', path: 'workspace-dir', name: 'workspace-dir' };
        await appState.commands.execute('file.openPath', 'subfolder/doc.md');
