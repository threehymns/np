...
async function readPersistenceFile() {
...
async function writePersistenceFile(data) {
    const filePath = getAppDataPath();
    try {
...
function registerIpcHandlers() {
    ...
    ipcMain.handle('fs:deleteEntry', async (_, entryPath) => {
        await fs.rm(entryPath, { recursive: true, force: true });
    });
    ipcMain.handle('fs:renameEntry', async (_, oldPath, newName) => {
        const newPath = path.join(path.dirname(oldPath), newName);
        await fs.rename(oldPath, newPath);
        return newPath;
    });
    // Git commands runner
    ipcMain.handle('git:run', async (_, workingDir, args) => {
